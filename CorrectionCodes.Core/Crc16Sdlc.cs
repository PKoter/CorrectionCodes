...
{
	public sealed class Crc16Sdlc : IByteBasedCode
	{
		static ushort[] _table = new ushort[]
		{
		...
		public static ushort ComputeChecksum(byte[] data, ushort seed = 0xffff, ushort final = 0)
		{
			uint crc = seed;
			...
			for (int i = 0; i < data.Length; i += 1)
			{
			   ...
			   uint index = (data[i] ^ (crc >> 8)) & 0xff;
			   crc = _table[index] ^ (crc << 8);
			}
			crc = (ushort)(crc ^ final);
			// this is affected by endianess i suppose, coz BitConverter returns bytes starting from least significant byte
